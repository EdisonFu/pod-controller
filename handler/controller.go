...
func SendPodNum(podNum int32) (err error) {
	...
	replica, err := clientSet.AppsV1().Deployments("default").GetScale(context.TODO(), "quant-k8s", metaV1.GetOptions{})
	replica.Spec.Replicas = podNum
	replica, err = clientSet.AppsV1().Deployments("default").UpdateScale(context.TODO(), "quant-k8s", replica, metaV1.UpdateOptions{})
	if err != nil {
		l4g.Error("put deployment error:%v", err)
	} else {
		l4g.Info("put deployment to:%d success", podNum)
	}
